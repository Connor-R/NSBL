    ...
    for index in range(4, 34):
        ...
        for row in range(8,max_row):
            ...
            if team_sheet.cell(row, 2).value not in ('Year','') and team_sheet.cell(row, 3).value not in ('Salary', ''):
                ...
                entry = {'player_name':player_name, "fname":first_name, "lname":last_name, "team_abb":primary_abb,  "position":position, "salary":salary, "year":year, "expires":expires, "opt":opt, "NTC":NTC, "salary_counted":salary_counted}
